...
class Trainer(object):
    def __init__(self, config, args):
        ...
        self.config = config
        self.args = args
        self.log_folder_name, self.log_dir, self.ckpt_dir = set_log(self.config, self.args.condition)
        self.seed = load_seed(self.config.seed)
        self.device = load_device()
        self.train_loader, self.test_loader = load_data(self.config, args.condition)
    ...
    def train(self, ts):
        ...
        for epoch in trange(0, (self.config.train.num_epochs), desc='[Epoch]', position=1, leave=False):
            ...
            for _, train_b in enumerate(self.train_loader):
                ...
                x, adj, con = load_batch(train_b, self.device)
                loss_subject = (x, adj, con) 
            ...
            for _, test_b in enumerate(self.test_loader):
                x, adj, con = load_batch(test_b, self.device)
                loss_subject = (x, adj, con)
            ...
            directory = f"./checkpoints/{self.config.data.data}/{self.args.condition}"
            ...
            if epoch % self.config.train.save_interval == self.config.train.save_interval - 1:
                ...
                }, f'./checkpoints/{self.config.data.data}/{self.args.condition}/{self.ckpt + save_name}.pth')
        ...
        logger.log(f'Train time: {runtime}')
