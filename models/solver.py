...
from models.losses import get_score_fn
...
class ReverseDiffusionPredictor(Predictor):
    ...
    def update_fn(self, x, adj, flags, t, condition): 

        if self.obj == 'x':
            f, G = self.rsde.discretize(x, adj, flags, t, condition, is_adj=False)  # →
            z = gen_noise(x, flags, sym=False)
            x_mean = x - f
            ...
            f, G = self.rsde.discretize(x, adj, flags, t, condition, is_adj=True)
            z = gen_noise(adj, flags)
            adj_mean = adj - f
    ...
    def update_fn(self, x, adj, flags, t, condition):
        ...
        if self.obj == 'x':
            for i in range(n_steps):
                grad = score_fn(x, adj, flags, t, condition)   
...
def get_pc_sampler(sde_x, sde_adj, shape_x, shape_adj, predictor='Euler', corrector='None',
    ...
    def pc_sampler(model_x, model_adj, init_flags, condition_X, condition_A): 
        ...
        with torch.no_grad():
            ...
            for i in trange(0, (diff_steps), desc='[Sampling]', position=1, leave=False):
                ...
                _x = x
                x, x_mean = corrector_obj_x.update_fn(x, adj, flags, vec_t, condition_X) 
                adj, adj_mean = corrector_obj_adj.update_fn(_x, adj, flags, vec_t, condition_A) 

                _x = x
                x, x_mean = predictor_obj_x.update_fn(x, adj, flags, vec_t, condition_X)  
                adj, adj_mean = predictor_obj_adj.update_fn(_x, adj, flags, vec_t, condition_A)  
            print(' ')
