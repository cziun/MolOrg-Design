...
def get_score_fn(sde, model, train=True, continuous=True):
    ...
    if isinstance(sde, VPSDE):
        def score_fn(x, adj, flags, t, condition):
            # Scale neural network output by standard deviation and flip sign
            if continuous:
                score = model_fn(x, adj, flags, condition) 
...
def get_sde_loss_fn(sde_x, sde_adj, train=True, reduce_mean=False, continuous=True,
    ...
    def loss_fn(model_x, model_adj, x, adj, condition):
        ...
        score_x = score_fn_x(perturbed_x, perturbed_adj, flags, t, condition)
        score_adj = score_fn_adj(perturbed_x, perturbed_adj, flags, t, condition)
