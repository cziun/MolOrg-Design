...
from models.sde import VPSDE, VESDE
from models.losses import get_sde_loss_fn
from models.solver import get_pc_sampler
...
def load_data(config, condition, get_graph_list=False):
    return dataloader(config, condition, get_graph_list)
...
def load_model_params(config):
    ...
    params_x = {'model_type':config_m.x, 'max_feat_num':max_feat_num, 'depth':config_m.depth, 'nhid':config_m.nhid, 
                    'condition_dim': config_m.condition_dim}
    params_adj = {'model_type':config_m.adj, 'max_feat_num':max_feat_num, 'max_node_num':config.data.max_node_num,
                    ...
                    'adim':config_m.adim, 'condition_dim':config_m.condition_dim, 'num_heads':config_m.num_heads, 
                    'conv':config_m.conv}
...
def load_batch(batch, device):
    ...
    con_b = batch[2].to(device_id)
    return x_b, adj_b, con_b


def load_ckpt(ckpt_train_path, config, device, ts=None, return_ckpt=False):
    device_id = f'cuda:{device[0]}' if isinstance(device, list) else device
    ckpt_dict = {}
    if ts is not None:
        config.ckpt = ts
    path = f'./checkpoints/{config.data.data}/{ckpt_train_path}.pth'
